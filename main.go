...
func guildMemberAdd(s *discordgo.Session, m *discordgo.GuildMemberAdd) {
	if _, present := loadedUserData.Users[m.User.ID]; !present {
		...
		s.ChannelMessageSend(c.ID, fmt.Sprintf("Please send me `%sregister {first name} {last name} {grade #}` (e.g. `%sregister Jono Jenkens 12`)", loadedConfigData.Prefix, loadedConfigData.Prefix))
	}
}
